...
10. **🔴 บันทึกทุกอย่างใน `docs/` ให้กระชับ ตรงประเด็น เน้นข้อเท็จจริง — ไม่ใช่ diary**
    เขียนแค่: **ตัดสินใจอะไร** (+เหตุผลสั้น 1 บรรทัด) · **สเปค/ฟังก์ชัน/schema ที่ทำจริง** · **ปัญหาที่ชัดเจนพร้อมทางแก้**
    ห้าม: เล่าลำดับการดีบัก ("ลองวิธี A แล้วพบว่า...", "เจอจริงตอนแก้ข้อ...") · เหตุผลประกอบเกินจำเป็น · ซ้ำเนื้อหาข้ามไฟล์ (อ้าง `PT-xx`/`D-xx` แทนคัดลอกซ้ำ) · ประโยคยืนยัน/อารมณ์ที่ไม่ใช่ข้อมูล
    ใช้กับทุกไฟล์ใน `docs/` โดยเฉพาะ `DECISIONS.md`/`PATTERNS.md`/`STATUS.md` — ก่อน commit ให้อ่านทวนว่าตัดออกได้อีกไหมโดยไม่เสียข้อเท็จจริง
11. **🔴 หาคำตอบไม่เจอจากแหล่งจริง (schema/inspect/ไฟล์) → บอกตรง ๆ ว่าไม่เจอ ห้ามเดาชื่อ/ค่าที่ "ฟังดูน่าจะใช่"** โดยเฉพาะตอนหา field/key ที่ชื่อคล้ายกันหลายตัว — เดาแล้วผิดจะดูมั่นใจและกลมกลืนจนตรวจจับยาก (เคยพลาดจริงที่ D-58: เดา key จาก grep ผลรวมหลายบรรทัดแทนอ่านไฟล์ตรง ๆ ทำให้ผูก URL เข้าผิด field) ถ้าไม่ชัวร์ให้ query/`Read` ไฟล์นั้นตรง ๆ ซ้ำ หรือถามคนแทนการเดา
