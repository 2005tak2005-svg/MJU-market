...
5. `lower(email)` ใช้ตอนเทียบ แต่ `handle_new_user()` ไม่ได้ `lower()` ก่อน derive — อีเมลที่มีตัวใหญ่ (เช่น `MJU6512345678@mju.ac.th`) จะ derive ไม่ออกแล้วได้ NULL ทั้งที่ควรได้เลข **ยังไม่ได้ทดสอบ**
