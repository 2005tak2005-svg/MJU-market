...
## D-81 — Wishlist / บันทึกสินค้าไว้ดูทีหลัง (2026-09-02)

**บริบท:** pete ขอ brainstorm ฟีเจอร์ที่ทำไม่ยากและเข้ากับสถาปัตยกรรมเดิม เลือก wishlist — precedent เกือบเหมือนเป๊ะคือ `advertisement_likes` (D-58: junction table + view คำนวณ `EXISTS(...auth.uid())`) แค่เปลี่ยนเป้าหมายจากประกาศแอดมินเป็นสินค้า ยืนยันแล้วว่าไม่เคยมีฟีเจอร์นี้อยู่ในเอกสาร/โค้ดที่ไหนมาก่อน

**ตัดสินใจ (pete ยืนยัน):** wishlist เป็น**ข้อมูลส่วนตัว** ไม่มี public count แบบ `like_count` — เพราะฉะนั้น RLS `SELECT` ของ `wishlist_items` จำกัดเฉพาะแถวตัวเอง (`user_id = auth.uid()`) ต่างจาก `advertisement_likes` ที่ `SELECT true` (ทุกคนเห็น เพื่อให้ aggregate count ทำงาน) ไม่มี RESTRICTIVE ban policy (มาตรฐานเดียวกับ `advertisement_likes` — บันทึก/เอาออกจาก wishlist เป็น action เบา ไม่ใช่การสร้างเนื้อหา)

**Supabase (apply ตรงผ่าน MCP ตามกฎข้อ 5 — ตารางใหม่ว่างเปล่า):**
- ตาราง `wishlist_items` (`product_id`, `user_id` composite PK, FK `products`/`Profile` ทั้งคู่ `ON DELETE CASCADE`) + RLS 3 policy (select/insert/delete เฉพาะแถวตัวเอง ไม่มี UPDATE — toggle ด้วย insert/delete)
- `products_review_view` เพิ่มคอลัมน์ `saved_by_me` (`EXISTS` inline, ไม่ต้อง LEFT JOIN เพราะเป็น existence check ไม่ใช่ aggregate) — `search_products` RPC ไม่ต้องแก้เลย (ยืนยันจาก `pg_get_functiondef` ว่า body เป็น `SELECT * FROM products_review_view` ตรง ๆ)
- ยืนยันด้วย SQL impersonation จริง (ไม่ใช่แค่อ่านนโยบาย): insert/delete แถวของตัวเองผ่าน, insert/delete แถวคนอื่นถูกบล็อกจริง (RLS `USING`/`WITH CHECK`), SELECT เห็นเฉพาะแถวตัวเอง, cascade delete จาก `products` ลบ `wishlist_items` ที่เกี่ยวข้องจริง (`orphaned_rows = 0`) — เพิ่ม check block `[3.6]`-`[3.9]` ใน `checks/L3.sql`

**FlutterFlow (7 push แยกตามข้อบังคับ same-push-register-then-reference ของโปรเจกต์):**
1. register `wishlist_items` + `products_review_view.saved_by_me` เข้า typed compiler (`app.raw` guarded find-or-add — เหลือไว้ในสคริปต์ ไม่ต้อง retire เพราะ idempotent เหมือน `advertisement_likes`'s registration เดิม)
2. `ProductGridSection` (custom widget, self-fetching, D-68 v2/D-75): เพิ่ม heart icon ต่อการ์ด (`Stack`+`Positioned` มุมบนขวาของรูป, nested `InkWell` ไม่ bubble ไปโดนการ์ดทั้งใบ — เทคนิคเดียวกับ D-76) mutate `product.savedByMe` ในหน่วยความจำแล้ว `setState` ทันที **ไม่ refetch** (ต่างจาก `advertisement_likes` ที่ต้อง full-refetch เพราะเป็น DSL-bound ListView — ที่นี่เป็น custom Dart ล้วน ควบคุมเต็มที่ + row model มี setter จริงทุกคอลัมน์)
3. `ProductDetails`: heart button ใหม่ (custom widget `WishlistToggleButton`) ข้าง `RateSellerButton` — **ลองผูก `initialSaved` param เข้ากับ `saved_by_me` แบบ dynamic ก่อนแล้วพังตอน compile** (`CustomWidget(...).arguments` รับแค่ `DslExpression`/literal เท่านั้น ไม่มี `DslExpression` subtype สำหรับ nodeKeyRef-scoped Postgres field ในเวอร์ชัน SDK นี้ — ช่องว่างเดียวกับที่ `RateSellerButton`'s `visible:` เจอมาก่อนจน D-64 ต้องหลบไป raw-proto `ensureVisibility()`, แต่ custom widget arguments ไม่มี raw-proto escape hatch แบบนั้น) แก้ด้วยให้ widget **self-fetch สถานะเริ่มต้นเอง** (query `wishlist_items` ตรงใน `initState`) แทนรับเป็น param — รับแค่ `productId` (ผ่าน `Param('productId')`, proven-safe) ไม่ต้อง refresh หน้าเลยเพราะหน้านี้เป็น scaffold-level `databaseRequest`/`FutureBuilder` (D-44/PT-26) ที่ typed action chain สั่ง rebuild ไม่ได้ (`RateSellerSheet` เลี่ยงปัญหานี้ได้เพราะเป็น custom code เรียก `safeSetState()` ตรง ไม่ใช่ typed action)
4. หน้า `MyWishlist` ใหม่ 3 push (bare shell → state/onLoad/body จริงผ่าน `ensureReplaced` ต่อยอด anchor ที่ตั้งชื่อเอง `Container(name: 'MyWishlistBody')` แทนต้องเดา auto-generated key → pull-to-refresh ผ่าน `page.ensureActions(..., ON_PULL_TO_REFRESH)` มิเรอร์ `chatList` เป๊ะ) — page-level typed query filter `saved_by_me = true` แบบเดียวกับที่ `Mypost` filter `seller_id` ไม่ต้องมี RPC/custom widget ใหม่เลย
5. `Home` header: ปุ่มไอคอนที่ 2 (`favorite_border`) ต่อจาก `NotificationsBellButton` → Navigate ไป `MyWishlist` — **ตัวแรกของฝั่ง user ที่มีไอคอนที่ 2 ในสล็อตนี้** (ฝั่ง user ไม่เคยมี Drawer/bottom nav มาก่อน มีแค่ `HomeAdmin`)

**🔴 เจอ + แก้ปัญหาที่ไม่เกี่ยวกับ wishlist ระหว่างทาง (บล็อกทุก push ไม่ใช่แค่ของ session นี้):** push แรกสุดของ session นี้ (แค่ register table เฉยๆ) ชน validation error เดิม (`Parameter "productId"... not properly set` / `Generator variable does not exist` ×3 / `Condition configuration is invalid`) ที่ไม่เกี่ยวกับ wishlist เลย — ไล่ดูพบว่าเป็น `ensureReplaced` เก่าของ `MyPostsList` (บล็อกที่เขียนไว้ตั้งแต่ก่อน D-78) ที่ไม่เคย retire หลัง D-78 (2026-08-30) แทนที่ itemBuilder เดิมด้วย `MyPostSwipeableRow` ไปแล้วจริง — anchor key `ListView_8nazfbk1` ไม่มีอยู่จริงในโปรเจกต์เลย (`inspect --page Mypost` สดยืนยัน key จริงคือ `ListView_oru8qeq4` ตรงกับที่ D-78 บันทึกไว้) ทำให้ `ensureReplaced` สร้าง subtree ใหม่แบบไม่มี generator scope ทุกครั้งที่รัน สคริปต์ท้องถิ่น (`lib/flutterflow_project/pages/mypost.dart`) เองก็ค้าง key เก่า (`ListView_px04ibmd`) ไม่ตรงกับทั้งของจริงและที่ D-78 บันทึกไว้ — **แปลว่าทุกการ push (ไม่ว่าเรื่องอะไร) พังมาตั้งแต่ D-78 landed จนถึงตอนนี้ ถ้าไม่มีใครสังเกต** ลบบล็อกทิ้งทั้งก้อนตามธรรมเนียม PT-16/21 (ของที่ landed แล้วต้อง retire ไม่ใช่ทิ้งไว้ให้รันซ้ำ) ยืนยันด้วย `validate` ผ่านทันทีหลังลบ

**ยังไม่ทำ:** ยังไม่ได้ทดสอบผ่านแอปจริงโดย pete ทั้งหมด (heart toggle บน Home grid/ProductDetails, หน้า `MyWishlist`, ปุ่มใน header) · ไม่มี public wishlist count (ตัดสินใจแล้วว่าไม่ทำ) · ไม่มีการแจ้งเตือนเมื่อสินค้าที่บันทึกไว้ถูกขาย/ลดราคา (ไม่ได้ขอ)

## D-82 — Mini Storefront + Badge ผู้ขายน่าเชื่อถือ (2026-09-03)

**บริบท:** ต่อยอดจาก brainstorm เดิม (D-81 ทำ wishlist ไปแล้ว) pete เลือกทำอีก 2 ฟีเจอร์: (2) หน้าดูสินค้าอื่น ๆ ของผู้ขายคนเดิม (mini storefront) (3) badge บอกผู้ขายน่าเชื่อถือ — ขอดูแผนก่อนลงมือ (Plan Mode) แล้วอนุมัติ ตัดสินใจไม่แบ่ง 2 เอเจนต์ทำคู่ขนานเพราะทั้งคู่แก้ `dsl/edit.dart` ไฟล์เดียวกัน/push โปรเจกต์เดียวกัน เสี่ยงชนแบบเดียวกับบั๊ก Mypost/D-78

**เกณฑ์ badge (pete ยืนยัน):** `is_trusted_seller := seller_avg_rating >= 4.5 AND seller_review_count >= 3`

**Supabase (apply ตรงผ่าน MCP ตามกฎข้อ 5 — เพิ่มคอลัมน์ boolean บน view ที่มีอยู่ ปลอดภัย):**
- `products_review_view` เพิ่มคอลัมน์ `is_trusted_seller` (`COALESCE(rv.avg_rating >= 4.5 AND rv.review_count >= 3, false)` — ใช้ `rv.*` ตรง ไม่ใช่ alias `seller_avg_rating`/`seller_review_count` เพราะ Postgres ไม่ให้อ้าง SELECT-list alias อื่นในบรรทัดเดียวกัน)
- ไม่มีตารางใหม่ ไม่มี RPC ใหม่ — `search_products` ไม่ต้องแก้ (RPC คืน `SETOF products_review_view` ตรง ๆ อยู่แล้ว)

**FlutterFlow (6 push):**
1. **เจอ + แก้บั๊กค้าง (ไม่เกี่ยวกับ D-82):** `ReportUserButton` (D-65) landed จริงแล้ว (`Button_j2vzor7s`) แต่ `component.ensureInsertedAfter` ยังไม่ถูก retire จาก `dsl/edit.dart` — ค้างเป็น one-shot ที่จะพังทุก push ถัดไป (คลาสเดียวกับบั๊ก Mypost/D-78 ที่เจอใน D-81) แก้โดย retire เป็นคอมเมนต์ก่อนแตะอะไรอื่น
2. register `is_trusted_seller` เข้า typed compiler (`app.raw` guarded find-or-add, idempotent เหมือน `saved_by_me`)
3. หน้า `SellerStorefront` (route `seller-storefront`) 3 push (shell → params `sellerId`/`excludeProductId` + state + query + body ผ่าน `ensureReplaced` → pull-to-refresh ผ่าน `page.ensureActions`) — page-level typed query filter `seller_id = param AND moderation_status='approved' AND status<>'sold' AND id<>excludeProductId` (public-facing ต่างจาก `Mypost` ที่โชว์ของเจ้าของเองรวม pending/rejected) มิเรอร์ pattern เดียวกับ `MyWishlist`/D-81 เป๊ะ
4. **Badge** — `visible:` ใน DSL รองรับแค่ `Equals`/`Not` ไม่มี comparator ตัวเลข (`>=`) ยืนยันจากอ่าน `references.dart` ตรง ๆ ตาม PT-24 §1 บังคับต้องคอมพิวต์ boolean ที่ SQL แล้วผูก `Equals(item['is_trusted_seller'], true)` — ทำ 2 จุด: `ProductDetails` (re-author `ProductDetailsContent` itemBuilder ทั้งก้อนตาม PT-23 §1, จุดเสี่ยงสูงสุด) และ `ProductGridSection` (raw Dart, `if (product.isTrustedSeller == true)` ต่อท้ายแถวดาวเดิม)
5. **เจอ + แก้บั๊กค้างอีกจุดระหว่าง re-author `ProductDetailsContent`:** มี raw-proto workaround เดิม (2026-08-29) ผูก tap-to-call เบอร์โทรผ่าน `page.findByKey('Text_ngs9a3px')` + `varFromGeneratorVariable` เพราะตอนนั้น widget ไม่ได้ author สดในรอบนั้น — พอ re-author itemBuilder ใหม่ทั้งก้อนสำหรับ badge (ทำให้กลายเป็น "author สด" จริง) key เดิมหายไป ทำให้ push แรกพังด้วย `findByKey("Text_ngs9a3px") found no matches` แก้โดยลบ raw-proto block ทิ้ง แล้วผูก `onTap: CallNumber(item['contact_phone'])` ตรงในทรีใหม่แทน (`item[]` sugar ใช้ได้ปกติเพราะเป็นกรณี author สดจริงแล้ว) — ฟีเจอร์เดิม (แตะเบอร์แล้วโทรออก) ยังทำงานเหมือนเดิม แค่เปลี่ยนกลไก
6. ปุ่มทางเข้า storefront ("ดูสินค้าอื่น ๆ ของผู้ขาย") ใน `UserProfileCard` ต่อจาก `ReportUserButton` — ขยาย `openProfileChain` ให้รับ `excludeProductId` optional (reset ทุก call site เหมือน field อื่น ตั้งค่าจริงเฉพาะจาก `ProductDetails`) เก็บใน App State ใหม่ `viewedProfileExcludeProductId` ใช้ได้จากทุกที่ที่เปิด popup นี้ (ProductDetails/BannedUsers/UserDirectory) — สมเหตุสมผลเพราะ D-52 คุม visibility ของสินค้าคนโดนแบนที่ view อยู่แล้ว
