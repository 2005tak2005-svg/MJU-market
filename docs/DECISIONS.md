...
## D-17 — Confirm Email เปิดอยู่จริง — ยังไม่ตัดสินใจว่าจะรับมือยังไง (2026-08-09)
...
**ทางเลือกที่ยังไม่ตัดสินใจ (pete เป็นคนเลือก ไม่ใช่ Claude ตัดสินเอง เพราะเป็น security trade-off):**

| ทางเลือก | ข้อดี | ข้อเสีย |
|---|---|---|
| **(ก) สร้าง flow ยืนยันอีเมลจริง** | ยังพิสูจน์ตัวตนผ่านอีเมล `@mju.ac.th` จริง ตรงตามจุดประสงค์เดิมของโปรเจกต์ | ต้องทำหน้าแจ้งเตือน + ดัก error เฉพาะ + อาจต้องมีปุ่ม resend — งานเพิ่มอีกก้อน |
| **(ข) ปิด Confirm Email ใน Supabase Dashboard** (Authentication → Sign In / Providers → Email) | สมัครเสร็จมี session ทันที ไม่ต้องมีหน้ายืนยันเลย ปิด L1 ได้เร็วที่สุด | **ใครก็สมัครด้วยอีเมล `@mju.ac.th` ของคนอื่นได้** — การยืนยันตัวตนผ่านอีเมลมหาลัยจะไม่เหลือความหมาย |

**Testing workaround ที่ใช้อยู่ตอนนี้ (ไม่ใช่ทางแก้ถาวร):** SQL patch `auth.users.email_confirmed_at` ตรงบัญชีทดสอบเป็นรายตัว — ดูรายชื่อบัญชีที่โดน patch ใน `STATUS.md` หัวข้อหนี้ทางเทคนิค **ห้ามเข้าใจผิดว่านี่คือทางแก้จริง**
