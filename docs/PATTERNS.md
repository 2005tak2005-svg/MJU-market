...
## PT-40 — `Icon.icon`/สีไม่ bind แบบ dynamic ได้เลยใน typed DSL + splice action node ออกจากกลางเชนต้อง guard ด้วยเนื้อหา (D-75, 2026-08-28)

**1. ไม่มีทางสร้าง "แถวดาวแตะเลือกได้" ด้วย native FF widget ตรงๆ** — `Icon(this.icon, ...)` รับ `icon` เป็น `String` literal (ไม่ใช่ `DslExpression`) เหมือนกับ `color`/glyph อื่นๆ ที่เป็น literal-only ทั้งหมดในกลุ่มนี้ — ไม่มีทาง bind ให้เปลี่ยนตามค่า state ได้เลยผ่าน typed DSL (ต่างจาก `Text`/`visible:` ที่ผูก expression ได้) FFWidgetType `RatingBar` มีอยู่จริง (ตั้ง literal ผ่าน fast-lane patch ได้ — `iconCount`/`initialRating`/`unratedColor`) แต่ก็ยังเป็น literal-only เหมือนกัน ไม่ช่วยเรื่อง dynamic tap-to-select **ทางแก้ที่ใช้ได้จริง:** custom Flutter widget (เหมือน `ProductGridSection`, PT-36) — เขียน state/สี/ไอคอนแบบ dynamic ได้อิสระเพราะเป็น Dart ธรรมดา ไม่ผ่านตัว compiler ของ DSL เลย
