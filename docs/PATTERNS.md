...
**1. `SetState('field', CustomFunction(...))` ใช้กับ page-state field ชนิด `List<PostgresRow>` ไม่ได้** — compile ผ่านเสมอ (`CustomFunctionInvocation` compile เป็น `FFVariable(source: FUNCTION_CALL, ...)` ถูก type ตามที่ประกาศ) แต่ **push แล้ว validate fail ทุกครั้ง**: `Field "X" has an update value that is not properly set in Update App State action` — เกิดกับทุก widget ที่ SetState field เดียวกันพร้อมกัน ไม่ใช่แค่จุดที่เพิ่งแก้ ยืนยันสาเหตุด้วยการถอด `CustomFunction(...)` wrap ออกแล้ว push สะอาดทันที **สรุป:** field ชนิดนี้รับค่าจาก `SetState` ได้เฉพาะที่มาจาก `PostgresQuery`'s `ActionOutput` (source `POSTGRES_QUERY`) เท่านั้น แม้ custom function จะประกาศ return type ตรงกับ field เป๊ะก็ไม่ผ่าน — ไม่มีตัวอย่างที่ใช้งานได้ในโปรเจกต์นี้หรือ `references/` เลยที่ป้อน custom-function result เข้า list-typed SetState **ถ้าต้อง transform list ก่อนเก็บลง state แบบนี้ (เช่น สุ่มลำดับ) ต้องทำที่ SQL/view แทน** (เช่น เพิ่มคอลัมน์ `random()` แล้ว `ORDER BY` คอลัมน์นั้น) ไม่ใช่ custom function ฝั่ง Dart
...
**ใช้แล้วที่:** L3 `Home` — สุ่มลำดับสินค้า (D-45) ทั้ง onLoad, search/pull-to-refresh chain, และ 26 category chip ล้วนโดนทั้งสองข้อนี้ — เช็คก่อนใช้ custom function ป้อน list-typed state field ที่ไหนก็ตาม และก่อนแก้ `orderBys`/query payload ของ trigger เดิมที่เคย push แล้ว
