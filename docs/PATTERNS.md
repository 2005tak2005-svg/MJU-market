...
**ใช้แล้วที่:** L3 `Home` — สุ่มลำดับสินค้า (D-45), ค้นหาแบบ substring + trigram index (D-46) ทั้ง onLoad, search/pull-to-refresh chain, และ 26 category chip ล้วนโดนข้อ 1-2 — `wrapSearchPattern` ใช้ข้อ 3-4 — เช็คก่อนใช้ custom function ป้อน list-typed state field ที่ไหนก็ตาม, ก่อนแก้ `orderBys`/query payload ของ trigger เดิมที่เคย push แล้ว, และก่อนเขียน custom function body แบบไม่ null-check
