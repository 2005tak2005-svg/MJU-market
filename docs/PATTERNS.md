...
**3. ห้ามห่อ `ListView` (เป้าหมายของ `ensureReplaced`) ด้วย `Expanded(...)`/`Flexible(...)` เพื่อกัน unbounded-height — ใช้ `shrinkWrap: true` บน `ListView` เองแทน (พบทำ D-55, 2026-08-22)**

เจอตอนต่อ tap-to-open-popup เข้า `BannedUsersList`: `page.ensureReplaced(page.findByKey('ListView_xxx'), Expanded(name: '...', ListView(...)))` — validate fail ด้วย `Field "viewedProfileUserId" has an update value that is not properly set` + `Generator variable does not exist` **ลามไปยัง itemBuilder อื่นที่ไม่เกี่ยวข้องเลยในหน้าอื่น** (`ChatListItems`/`SalesBySellerList`/`PendingProductsList`) ทั้งที่ไม่ได้แตะหน้าพวกนั้นเลยในพุชนั้น — สลับกลับเป็น bare `ListView(name: ..., shrinkWrap: true, ...)` (ไม่มี `Expanded` ห่อ) แก้ได้ทันที เข้าใจว่า `ensureReplaced`'s item-scope/generator-variable tracking อาศัยว่า `ListView` เป็น **root node ของ replacement โดยตรง** — ห่อด้วย wrapper อื่นแม้จะมี `name:` ให้ผ่านเงื่อนไข "root ต้องมีชื่อ" ก็ยังทำให้ tracking พังอยู่ดี

**ทางแก้:** ต้องการกัน unbounded-height ให้ใช้ `shrinkWrap: true` (ค่าใน `ListView` เอง, แพทเทิร์นเดียวกับที่ D-40 ใช้แก้ `chatList` ไปแล้ว) แทนการห่อ `Expanded`/`Flexible` เมื่อ `ListView` นั้นเป็นเป้าหมายของ `ensureReplaced`

**ใช้แล้วที่:** L8 (`HomeAdmin` — การ์ดผู้ใช้ถูกระงับ), D-55 (`BannedUsersList`) · เช็คทุกครั้งที่จะ "ผูกค่าเดียวแบบ scalar" ด้วยลูกเล่น ListView-1-item, จะรัน `ensureReplaced`/`ensureInsertedInto` ซ้ำในสคริปต์ที่เคย push สำเร็จมาก่อนแล้ว, หรือจะห่อ `ListView` เป้าหมายของ `ensureReplaced` ด้วย `Expanded`/`Flexible`
...
8. **การ re-author itemBuilder ทั้งก้อนใหม่ (เช่น เพิ่ม `visible:` badge) ทำให้ raw-proto workaround เดิมที่ผูกกับ static key ในทรีเก่าพังทันที** — ถ้าเคยมี `page.findByKey('X')` + `varFromGeneratorVariable(...)` ผูก action พิเศษไว้กับ widget ตัวใดตัวหนึ่ง**ข้างใน** itemBuilder ที่กำลังจะ re-author (เพราะตอนทำครั้งแรกยังไม่ใช่ "author สด" ตามข้อ 1) การ re-author รอบใหม่จะสร้าง key ใหม่ทั้งหมด ทำให้ `findByKey('X')` เดิม error `found no matches` ทันทีที่ push — **ไม่ใช่ error ที่เกี่ยวกับโค้ดที่เพิ่งเขียน** ต้องไล่หา `page.findByKey`/`app.raw` อื่นที่อ้าง key ในซับทรีเดิมก่อน push (ทั้งไฟล์ ไม่ใช่แค่จุดที่แก้) แล้ว**ลบ workaround ทิ้ง** ใช้ `item['field']`/`onTap:` ตรงในทรีใหม่แทน เพราะตอนนี้กลายเป็นกรณี "author สด" แล้วจริง ๆ (ใช้ `item[]` ได้ปกติ ไม่ต้องพึ่ง raw proto อีก) — เจอจริงที่ D-82 (`ProductDetailsContent` re-author เพื่อใส่ badge ทำให้ tap-to-call เบอร์โทรเดิมที่ผูกด้วย raw-proto พังกลางทาง)
...
## PT-30 — table/view ที่สร้างตรงใน Supabase ต้อง register เป็น typed table ก่อน DSL อ้างถึงได้ + outputAs collision ที่ "unique แล้วก็ยังพัง" (D-56, 2026-08-22)
...
**🔴 2 กับดักเจอตอนแก้โค้ด widget รอบถัดมา (Live Test Mode รายงาน analyzer error):**

1. **หน้า/page-state Integer field เป็น `int?` (nullable) เสมอ แม้ตั้ง `.withDefault(0)`** — ตรวจ `generated_code/lib/home/home_model.dart` แล้วเจอทั้ง `int? selectedCategoryId = 0;` และ `int? gridRefreshToken = 0;` (มีค่า default จริงแต่ type ยัง nullable) — ถ้า custom widget parameter ที่ผูกกับ field แบบนี้ประกาศ type เป็น `int` (non-nullable) จะ error `argument_type_not_assignable: The argument type 'int?' can't be assigned to the parameter type 'int'` **ไม่ถูกจับตอน `flutterflow ai run`/`compileDslApp` เลย** (แค่ validate proto/DSL shape ไม่ทำ full Dart analysis) ต้องเปิด FlutterFlow web IDE → Live Test Mode ถึงจะเห็น ก่อน declare parameter ที่ผูกกับ page/app state Integer field ให้เดา type เป็น `int?` ไว้ก่อนเสมอ เว้นแต่ตรวจ `generated_code/`/model จริงแล้วว่าไม่ nullable

2. **`app.customWidget(...)` แก้ `code:` ซ้ำไม่ได้ — throw ทันทีที่ payload เปลี่ยน** (compile ไปเป็น `ensureCustomWidget` ซึ่ง create-if-missing only) ต้องเปลี่ยนไปใช้ `app.raw((project) { ... })` + `custom_code_helpers.findCustomWidget`/`addCustomWidget`/`updateCustomWidget` แบบ find-first (pattern เดียวกับที่ `searchProducts` custom action ใช้อยู่แล้วในไฟล์นี้) ตั้งแต่แรกถ้ารู้ว่าต้องแก้โค้ดอีกแน่ๆ — และเมื่อเปลี่ยนมาใช้ `app.raw` แล้ว **การ "วาง" widget ต้องเปลี่ยนจาก callable handle (`app.customWidget(...)` คืนมา) ไปเป็น `CustomWidget(widgetName: '...', name: '...', arguments: {...})` ตรงๆ แทน** (`app.customWidget(...)`'s callable ใช้ไม่ได้อีกต่อไปเพราะไม่ได้เรียกฟังก์ชันนั้นแล้ว)

3. **`page.ensureReplaced(selection, CustomWidget(...))` ไม่ preserve key ของ selection เดิม** (ต่างจากที่เอกสารสรุปไว้ว่า "preserving key" — ใช้ได้กับการแทนที่ด้วย native widget เท่านั้น) `UI.customWidget()` เจนคีย์ใหม่แบบสุ่มทุกครั้ง (`generateNodeKey(FFWidgetType.Container)`) ไม่มีการ override ด้วยคีย์เดิม ยืนยันเชิงประจักษ์: หลัง push ครั้งแรก `findByKey('GridView_fxvqqbog')` (คีย์เดิมก่อนแทนที่) หาไม่เจอเลยในรอบถัดไป กลายเป็นคีย์สุ่มใหม่ (`Container_lg66lo2h`) แทน — **`ensureReplaced` เรียกได้แค่ครั้งเดียวตอน "แทนที่ native widget เดิมด้วย custom widget ครั้งแรก" เท่านั้น** พอแทนที่ไปแล้วต้องลบ call นั้นออกจากสคริปต์ทันที (เหมือน pattern MyPostsList ด้านบน) ไม่งั้น rerun ครั้งถัดไปจะ error "found no matches" ทุกที การแก้โค้ด widget ครั้งต่อๆ ไปใช้ `updateCustomWidget` เท่านั้น ไม่ต้องแตะ placement/`ensureReplaced` อีกเลย

---

## PT-37 — `addproduct` submit เงียบเมื่อไม่เลือกหมวดหมู่: root cause + วิธี reconstruct action chain ทั้งก้อนอย่างปลอดภัย (D-70, 2026-08-26)

**บั๊ก:** pete รายงาน "กดลงขายสินค้าบางทีก็ส่งถึงแอดมิน บางทีก็เงียบไม่มีสแนตช์บาร์" — เกิดเมื่อไม่เลือกหมวดหมู่จาก Dropdown เท่านั้น (ไม่เกี่ยวกับ title/price/condition — 3 ตัวนี้ตรวจแล้วมี fallback ที่ไม่ทำให้ insert throw)

**Root cause:** ปุ่มเดิมเช็คแค่ `title` ว่าง แล้ว insert ตรงเข้า `ProductsTable()` ไม่มี try/catch เลย `category_id` เป็น NOT NULL + FK → `"CAT"(id)` (SCHEMA.md) แต่ `parseCategoryId(null)` fallback เป็น `0` (ไม่มี id=0 ใน `"CAT"`, seed 1-12) → insert throw FK violation แบบไม่มีใครดักจับ → เงียบสนิท (ไม่มี error surface ให้ user เห็นเลย) ยืนยันจาก PT-18: `PostgresCreate` ไม่มี `onSuccess`/`onFailure` และ SDK เวอร์ชันนี้ไม่มี chain-level try/catch ให้เลย ทางแก้เดียวที่มีคือปิด root cause ฝั่ง client (แบบเดียวกับ D-24) ไม่ใช่ error handling

**🔴 `Equals(WidgetState(dropdown, .value), null)` ใช้ไม่ได้ — compile เป็น `_model.dropDownValue == ''` เสมอ (ขยาย PT-12 §11 จาก Postgres row field ไปถึง widget-state ด้วย)** runtime จริงตอนไม่แตะ Dropdown คือ `null` ไม่ใช่ `''` (ไม่มี default) เทียบกับ `''` แล้วได้ `false` เสมอ = guard ไม่ทำงานจริงทั้งที่ push ผ่านและดูถูกต้องใน `generated_code/` เผินๆ (ต้องเทียบ literal ที่ compile ออกมากับ default จริงของ field เสมอ ไม่ใช่แค่เช็คว่า compile ผ่าน) **ทางแก้:** เทียบผลลัพธ์ของ custom function (`parseCategoryId(...)`) กับ sentinel `0` ที่มันคืนจริงแทน — ทำงานในโดเมน int ไม่ใช่ string/null จึงเลี่ยง quirk นี้ไปได้เลย: `Equals(CustomFunction(parseCategoryIdFn, args: {...}), 0)` compile เป็น `functions.parseCategoryId(_model.dropDownValue) == 0` ตรงตามต้องการ

**วิธี reconstruct action chain ทั้งก้อนอย่างปลอดภัยเมื่อบางส่วนใช้ typed DSL ไม่ได้:** ปุ่มนี้มีขั้นตอนแรก (`selectedConditionLabel = choiceChipsValue`) ที่ต้องอ่านค่าจาก `ChoiceChips` ซึ่ง `WidgetState` ไม่รองรับ (PT-12 §5) แต่ `ensureActions` REPLACE ทั้ง trigger เสมอ (ไม่ใช่ append) จึงต้อง reconstruct ทั้งก้อน ใช้ 2 ขั้นตอน:
1. Author ทั้งชุดผ่าน typed DSL (`If`/`Snackbar`/`PostgresCreate`/`CustomFunction`/`SetState`) โดยขั้นตอนแรกใส่ placeholder self-assignment (`SetState(field, State(field))`) ไปก่อน
2. `app.raw` แยกอีกก้อน หา `trigger.rootAction` (= placeholder ตัวเดียวกัน เพราะเป็น action แรกในลิสต์) แล้ว overwrite `.action.localStateUpdate.updates[0].setValue = FFValue(variable: variable_helpers.varFromWidgetValue(nodeKey, data_type_helpers.stringType))` ตรงๆ — proto shape เดียวกับที่ ProfileUser avatar capture ใช้อยู่แล้ว (PT-12 §1/§2)
...
## PT-39 — สอง action ที่ structure เหมือนกันเป๊ะใน widget เดียวชนกัน (ไม่ใช่แค่ชื่อซ้ำ) + `Equals(item['x'], null)` ไม่ตรวจ null จริง (D-74, 2026-08-28)

**1. Action สอง node ที่ structure เหมือนกันทุกจุดใน action tree เดียวกัน ชน validate แม้ `outputAs`/ไม่มี `outputAs` เลยก็ตาม** — ลองเขียน `If(type=='listing_approved', then: [Navigate(...)], orElse: If(type=='listing_rejected', then: [Navigate(...เหมือนเดิมทุกตัวอักษร...)]))` (สอง `Navigate` เหมือนกันเป๊ะ คนละ branch) ได้ error เดียวกับ PT-38 เป๊ะ ("output variable with the same name as another widget") ทั้งที่ `Navigate` ไม่มี `outputAs` ให้ตั้งด้วยซ้ำ — เปลี่ยนชื่อ `outputAs` ของ action อื่นในเชนไม่ช่วย ลบเชนอื่นออกจนเหลือแค่ 2 `Navigate` นี้คู่เดียวก็ยังพัง พิสูจน์ด้วยการลบ `Navigate` ตัวที่สองออกแล้ว validate ผ่านทันที **สรุป: ปัญหาไม่ใช่ชื่อซ้ำ แต่เป็น structural duplication ของ action node สอง node ในต้นไม้เดียวกัน (ไม่ว่าจะมี output หรือไม่)**

**ทางแก้:** อย่าให้มี action ที่ structure เหมือนกันซ้ำสองจุดในต้นไม้เดียว — ถ้าตรรกะเดิมคือ "ทำ X เมื่อเงื่อนไข A หรือเงื่อนไข B" (ต้องการ OR แต่ DSL นี้ไม่มี `Or`) ให้กลับด้าน เช็คนิเสธของเงื่อนไขที่ไม่ต้องการแทนแล้ว nest แบบ AND (`If(Not(A'), then: [If(Not(B'), then: [X])])`) จะได้ `X` (action ที่ต้องการ) อยู่จุดเดียวในต้นไม้ ไม่ซ้ำ — ใช้ได้จริงเมื่อ type มีค่าที่เป็นไปได้จำกัด (4 ค่า, 2 ค่าที่ไม่ต้องการ)

**2. `Equals(item['field'], null)` ไม่ได้ compile เป็นการเช็ค null จริง — เป็นการเทียบกับ `''` เสมอ (ขยาย PT-37 จาก widget-state ไปถึง item-scoped Postgres row field ด้วย)** — เขียน `Not(Equals(item['ref_product_id'], null))` เพื่อกัน Navigate ตอน field เป็น null ได้ push ผ่านและดู "ถูกต้อง" ใน DSL แต่ตรวจ `generated_code/` แล้วเจอ compile เป็น `!(notificationItem.refProductId == '')` — field ที่เป็น `String?` จริง (`getField<String>('ref_product_id')` ไม่มี default) ตอนเป็น null จริงจะไม่เท่ากับ `''` เลย (`null == ''` เป็น `false` เสมอ) ทำให้ guard "ไม่บล็อกอะไรเลย" ตรงข้ามกับที่ตั้งใจ **กฎเดิม (PT-37) ใช้กับ item-scoped field ด้วย ไม่ใช่แค่ widget-state:** ห้ามเช็ค null ของ nullable field ผ่าน `Equals(x, null)` เลย ให้เช็คผ่านฟิลด์อื่นที่ไม่มีทาง null แทน (เช่นในเคสนี้ ใช้ `type` ซึ่ง NOT NULL แทน `ref_product_id` ซึ่ง nullable — ได้ผลลัพธ์ตรงกันเพราะ `ref_product_id` non-null ก็ต่อเมื่อ `type` เป็น 2 ค่านั้นเท่านั้น) **ต้องตรวจ `generated_code/` เสมอหลัง push แม้ validate ผ่านและดู logic ถูกต้องบนกระดาษ** — เจอจากการอ่านโค้ด generate จริงเท่านั้น ไม่ใช่จาก validate/push สำเร็จ

**ใช้แล้วที่:** `Notifications`' row template (D-74) — เช็คทุกครั้งที่ต้องเขียน action ซ้ำกันสองจุดในต้นไม้เดียว (ให้กลับเป็น AND-nesting แทน) และทุกครั้งที่จะเช็ค null ของ Postgres row field ผ่าน `item[]` (ใช้ NOT NULL field แทนเสมอถ้าทำได้)

---

## PT-40 — `Icon.icon`/สีไม่ bind แบบ dynamic ได้เลยใน **typed DSL** (แต่ native widget เองทำได้ผ่าน raw proto) + splice action node ออกจากกลางเชนต้อง guard ด้วยเนื้อหา (D-75, 2026-08-28)

**1. `Icon(this.icon, ...)` (typed DSL) รับ `icon` เป็น `String` literal เท่านั้น — bind ให้เปลี่ยนตาม state ผ่าน typed DSL ไม่ได้เลย** (ต่างจาก `Text`/`visible:` ที่ผูก `DslExpression` ได้ตรงๆ) — เข้าใจผิดรอบแรกว่า FFWidgetType `RatingBar` ก็เป็น literal-only เหมือนกันเพราะเช็คแค่ `list_props`/fast-lane (ผิว literal-only ของ fast-lane เท่านั้น) **แก้แล้ว:** proto จริง (`FFRatingBar`) มี `allowInteraction: bool` (แตะเลือกได้จริง กลไกเดียวกับ Slider) + `initialRatingValue`/`unratedColorValue`/`emptyIconValue`/`fullIconValue` เป็น `*Value` type ที่ผูก `variable:` ได้ปกติ (`FFDoubleValue`/`FFColorValue`/`FFIconValue`) — **native `RatingBar` ทำ dynamic tap-to-select ได้จริง** แค่ต้องผ่าน raw proto (`node.props.ensureRatingBar()`) เพราะ typed DSL ไม่มี wrapper class ให้เท่านั้น ไม่ใช่ทำไม่ได้ในระดับ FFWidgetType **บทเรียน:** เจอ widget ที่ typed DSL ไม่ support ต้องเช็ค proto class จริง (`class FFXxx` ใน `flutterflow.pb.dart`) ก่อนสรุปว่า "ทำไม่ได้เลย" — `list_props`/fast-lane โชว์แค่ literal-valued properties ไม่ใช่ความสามารถทั้งหมดของ widget type นั้น **ทางเลือกที่ใช้จริง (D-75):** custom Flutter widget (เหมือน `ProductGridSection`, PT-36) — เขียน state/สี/ไอคอนแบบ dynamic ได้อิสระเพราะเป็น Dart ธรรมดา เลือกทางนี้เพราะของเดิมทำ+ทดสอบผ่านแล้วตอบโจทย์ ไม่ใช่เพราะ native ทำไม่ได้

**2. Splice action node ออกจากกลางเชนที่มีอยู่แล้ว ต้อง guard ด้วยเนื้อหาของ node ไม่ใช่ key ของมัน** — ต้องการลบ step กลางเชน (เดิมเซ็ต `pendingReviewRating` จาก widget state ของ Slider ที่ลบไปแล้ว) โดยไม่แตะ step อื่นเลย ทำโดย `firstNode.followUpAction = firstNode.followUpAction.followUpAction` (raw proto) — ถ้า guard การรันซ้ำด้วยการเทียบ `.key` ของ node ตรงๆ จะพังทันทีที่ push ครั้งถัดไป เพราะ key พวกนี้ไม่คงที่ (regenerate ทุกครั้งที่ widget ข้างเคียงถูก `ensureReplaced`) **ใช้ได้จริง:** guard ด้วยเช็คเนื้อหาแทน (`node.action.localStateUpdate.updates.first.fieldIdentifier.name == 'pendingReviewRating'`) — ชื่อ field เป้าหมายไม่เปลี่ยนแม้ key จะเปลี่ยน ทำให้ splice block ปลอดภัยที่จะเหลือไว้ในสคริปต์ตลอด (ต่างจาก `ensureReplaced` ที่ต้อง retire)

---

## PT-41 — Custom widget ไม่รองรับ parameter ชนิด `action` เลยใน SDK เวอร์ชันนี้ (D-78, 2026-08-29)

**`app.customWidget('X', parameters: {'onTap': action, ...}, ...)` compile ไม่ผ่านเสมอ** — `flutterflow ai validate` ฟ้องตรงๆ: `Unsupported operation: Action types are not compiled in declaration pass 1.` (ยืนยันจาก SDK source `compiler.dart`: `_compileType` throw ทันทีที่เจอ `ActionType` — ไม่ใช่ error message ที่บอกวิธีแก้ ต้องอ่าน source เอาเอง) **สำคัญ: จำกัดเฉพาะ custom widget (`app.customWidget`)** — `app.component(params: {'onTapAction': action})` ใช้ action param ได้ปกติ (`ParamAction('onTapAction')` ผูกใน body ได้ตรงๆ, ดู `shopflow_dsl.dart`) เป็นคนละ compile path กัน (component ผ่าน pass ที่รองรับ action, custom widget ตัน)

**ผลคือ custom widget เรียก native FlutterFlow action chain (Delete Row/Navigate/SetState) ผ่าน callback parameter จากภายนอกไม่ได้เลย** — ถ้าต้องการให้ custom widget ทำงานที่ปกติควรเป็น native action (ลบแถว, navigate, อัปเดต state) ต้องเลือกทางใดทางหนึ่ง:
1. **ทำทุกอย่างในตัว widget เองตรงๆ** (เหมือน `mark_product_sold`/`search_products`: `Supabase.instance.client...` ตรงๆ + `context.pushNamed(XWidget.routeName, ...)` — pattern ที่ generated code ของหน้าอื่นใช้อยู่แล้ว คัดลอกจาก `generated_code/lib/<page>/<page>_widget.dart` เดิมมาแทนเดา) — **ใช้จริงที่ `MyPostSwipeableRow` (D-78)**
2. หรือเปลี่ยนไปใช้ **component** แทน custom widget ถ้ายอมรับได้ว่าทำผ่าน typed DSL widget tree ล้วน (ไม่มี Dart ธรรมดา เช่นกรณีต้องการ `Dismissible`/gesture ที่ไม่มี native widget รองรับ ก็เลือกทางนี้ไม่ได้อยู่ดี)

**ผลข้างเคียงที่ต้องรับ (ทางเลือก 1):** custom widget ที่ mutate ข้อมูลเอง (เช่นลบแถว) **เข้าไม่ถึง page state ของหน้าที่มันอยู่** (ไม่มี `Provider`/`InheritedWidget` ห่อ `_model` ให้ — ตรวจ `generated_code/lib/<page>/<page>_widget.dart` แล้วไม่เจอ `ChangeNotifierProvider` เลยสักหน้า) ถ้าต้องการ "หายจากลิสต์ทันที" ให้จัดการด้วย **local state ของ widget เอง** (เช่น flag `_deleted` แล้ว render `SizedBox.shrink()`) ไม่ใช่พยายามเขียนกลับเข้า parent list — Dismissible เข้มงวดเรื่องนี้จริง (ต้องหายจากข้อมูลที่มันอ่านมาก่อนถูก rebuild ไม่งั้น assert) แต่ local state ก็พอสำหรับ Dismissible เพราะมันไม่ได้รีบิลด์ตัวเองจากภายนอกอีกถ้า parent ไม่ trigger — ปลอดภัยตราบใดที่ทุก refresh path ของหน้านั้นเป็น fresh query จาก DB จริง (ของเก่าที่ลบไปแล้วจะไม่โผล่กลับมาเพราะหายจริงฝั่ง DB)

**ใช้แล้วที่:** `RateSellerSheet`'s `StarRatingInput` (D-75) — เช็คก่อนพยายามทำ widget ที่ต้อง dynamic-bind สี/ไอคอน/glyph ใดๆ (ไปทาง custom widget ตั้งแต่แรก อย่าเสียเวลาลองใน typed DSL) และทุกครั้งที่ต้องแก้/ลบ step เดียวกลางเชน action ที่มีอยู่แล้วโดยไม่อยากรื้อทั้งก้อน

---

## PT-42 — junction table + view-level `EXISTS(...auth.uid())` toggle idiom (ใช้ครั้งที่ 3 แล้ว, D-81, 2026-09-02) + `CustomWidget(...).arguments` รับแค่ `DslExpression`/literal เท่านั้น

**1. Pattern สำเร็จรูปสำหรับ "ปุ่ม toggle ต่อ user ต่อ item" (ไลก์/บันทึก/ให้สิทธิ์):** junction table composite PK `(target_id, user_id)`, **ไม่มี UPDATE policy เลย — toggle ทำด้วย insert/delete เท่านั้น**, แล้วให้ view ที่ query จริง (ไม่ใช่ table ตรง ๆ) มีคอลัมน์ boolean คำนวณจาก:
- `EXISTS(...)` แบบ inline correlated subquery — พอสำหรับ existence-only flag (`liked_by_me`/`saved_by_me`)
- `LEFT JOIN (SELECT ... GROUP BY ...)` — ต้องใช้เมื่อต้องการ aggregate/scalar ด้วย (`like_count`, `seller_avg_rating`)

ใช้แล้ว 3 รอบ: `advertisement_likes`→`liked_by_me` (D-58, public select), `reviews`+`transactions`→`can_rate_seller` (D-64, computed boolean คล้ายกันแต่ผ่าน join สอง step), `wishlist_items`→`saved_by_me` (D-81, **select จำกัดแค่แถวตัวเอง** — ต่างจาก D-58 เพราะไม่มี public count ให้ต้องเปิดกว้าง) **จุดตัดสินใจ:** ถ้า RLS `SELECT` ของ junction table แคบกว่า `SELECT true` ก็ยังใช้ pattern นี้ได้ปกติ เพราะ view เป็น `security_invoker=true` และ `EXISTS` subquery ต้องการแค่ให้ผู้เรียกเห็นแถวของตัวเอง (ซึ่ง "select เฉพาะแถวตัวเอง" ก็ให้พอดี)

เพิ่มคอลัมน์เข้า view ที่ลงทะเบียนแล้วใน FF project ผ่าน `postgres_helpers.addTableField` (ไม่ใช่ `addTable` — ตัวนั้นสำหรับตาราง/view ที่ยังไม่เคยลงทะเบียนเลย) ตัวอย่างจริง: `dsl/edit.dart` บล็อก D-64 (`is_unread`/`has_message`) และ D-81 (`saved_by_me`)
