...
> ✅ **D-45: `Home` ค้นหา + สุ่มลำดับสินค้า + pull-to-refresh** — สุ่มฝั่ง Dart (custom function) ทำไม่ได้จริง (`SetState` ของ `List<PostgresRow>` field รับค่าจาก custom function ไม่ได้ — backend validate fail ทุกครั้งแม้ type ตรง) ย้ายไปสุ่มที่ SQL แทน: เพิ่ม `products_review_view.shuffle_key` (`random()`) แล้ว `ORDER BY shuffle_key` แทน `created_at` ทุก query (onLoad/search/26 category chip) 🔴 เจอกับดักใหม่ 2 ข้อ: (1) เปลี่ยน `orderBys` โดยใช้ `outputAs` เดิม push ผ่านแต่โค้ดที่ export ไม่เปลี่ยนเลย ต้องเปลี่ยน `outputAs` ด้วยเสมอ (2) ลืม retire `ensureReplaced` ของช่องค้นหาตาม PT-16/D-27 ทำให้พุชถัดมา error เฉพาะ widget นั้น แก้ด้วยย้ายไปผูก `onChanged`/`onSubmitted` ผ่าน `page.ensureActions` แทน ค้นหา (title, submit-only ไม่ query ทุก keystroke) กับ category chip เป็นคนละแกน กดอันหนึ่งล้างอีกอันทิ้ง 🟡 **ค้นหาเป็น exact match เท่านั้น ไม่ใช่ substring** (`iLike` ไม่มี `%...%` wrap ให้ DSL ไม่มี string-concat expression ที่ใช้กับ filter value ได้ — ยังไม่ปิด) รายละเอียด `DECISIONS.md` D-45, pattern ใหม่ `PATTERNS.md` PT-27 — **ยังไม่ได้ให้ pete ทดสอบผ่านแอปจริง**
...
8. **[D-45] ทดสอบ `Home` ค้นหา + สุ่มลำดับ + pull-to-refresh ผ่านแอปจริง** — ยังไม่เคยเปิดแอปดู เช็ค: ลำดับสินค้าเปลี่ยนทุกครั้งที่เข้าหน้า/pull-to-refresh, ค้นหาด้วยชื่อเป๊ะ (case-insensitive) เจอ, กด category chip ล้างช่องค้นหาจริง, สินค้า pending/rejected ไม่หลุดมาในผลค้นหา — 🟡 รู้อยู่แล้วว่าค้นหาเป็น exact match เท่านั้น ไม่ต้องรายงานซ้ำถ้าเจอ (ดู `layers/L3-browse-search.md` ค้างอยู่)
...
| 3 | Browse / Search / Filter | 🟨 เพิ่ม `shuffle_key` (`random()`) เข้า `products_review_view` แล้ว (D-45) | 🟨 `AllList`+`ProductDetails` ทำแล้ว · category filter chip requery จริงแล้ว (D-37) · `AllList` เป็น grid 2 คอลัมน์ + รูปจริงแล้ว (D-38) · `ProductDetails` โชว์รูปจริง + fallback icon แล้ว (D-42) · ค้นหา (exact match) + สุ่มลำดับ + pull-to-refresh เพิ่มแล้ว (D-45) — ยังไม่ทดสอบผ่านแอปจริงทั้งหมด | `Home` grid สินค้าไม่มีรูปยัง crash ได้ (`firstImageUrl!` ไม่มี fallback, ต่างจาก `ProductDetails` แล้ว) · ค้นหายังเป็น exact match ไม่ใช่ substring (D-45) · ยังไม่มีช่วงราคา |
...
**Layer 3** — ตอบแล้ว (D-45): built-in filter พอ ไม่สร้าง RPC `search_products` (P-05 ยังคงเป็นข้อเสนอค้าง) — คำถามใหม่ที่เหลือ: จะลงทุนแก้ค้นหาให้เป็น substring (ไม่ใช่ exact match) ด้วย raw-proto surgery ไหม หรือย้อนไปทำ P-05 ทีหลัง
