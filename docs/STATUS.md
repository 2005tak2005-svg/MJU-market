...
8. **[D-45/D-46] ทดสอบ `Home` ค้นหา + สุ่มลำดับ + pull-to-refresh ผ่านแอปจริง** — ยังไม่เคยเปิดแอปดู เช็ค: ลำดับสินค้าเปลี่ยนทุกครั้งที่เข้าหน้า/pull-to-refresh, ค้นหาด้วยคำบางส่วน (ไม่ใช่ชื่อเป๊ะ) เจอจริง, pull-to-refresh ตอนไม่ได้ค้นหาโชว์ของครบไม่ว่างเปล่า, กด category chip ล้างช่องค้นหาจริง, สินค้า pending/rejected ไม่หลุดมาในผลค้นหา — 🟡 รู้อยู่แล้วว่ายังไม่มี empty-state UI (ค้นหาไม่เจอ = grid ว่างเปล่าเฉย ๆ ) ไม่ต้องรายงานซ้ำถ้าเจอ (ดู `layers/L3-browse-search.md` ค้างอยู่)
...
| 3 | Browse / Search / Filter | 🟨 เพิ่ม `shuffle_key` (`random()`, D-45) + trigram index บน `products.title` (D-46) | 🟨 `AllList`+`ProductDetails` ทำแล้ว · category filter chip requery จริงแล้ว (D-37) · `AllList` เป็น grid 2 คอลัมน์ + รูปจริงแล้ว (D-38) · `ProductDetails` โชว์รูปจริง + fallback icon แล้ว (D-42) · ค้นหาแบบ substring + สุ่มลำดับ + pull-to-refresh เพิ่มแล้ว (D-45/D-46) — ยังไม่ทดสอบผ่านแอปจริงทั้งหมด | `Home` grid สินค้าไม่มีรูปยัง crash ได้ (`firstImageUrl!` ไม่มี fallback, ต่างจาก `ProductDetails` แล้ว) · ยังไม่มี empty-state UI (ลองแล้วไม่สำเร็จ D-46) · ยังไม่มีช่วงราคา |
